...
`SttService.live(config)` in `src/stt/service.ts` is the only product-facing entrypoint; it dispatches to `codexSttLayer` (`src/stt/codexRealtimeService.ts`) or `openRouterSttLayer` (`src/stt/openrouter.ts`) based on `config.provider`. Each provider file owns a single `Layer<SttService, ...>` built from its own free functions; there are no provider-level Context.Services. The `SttService` interface is stream-only (`transcribeStream` / `translateStream`); there is no clip surface.

The Codex layer is stream-native; its `transcribeStream` forwards the audio stream directly to `transcribeWithCodexRealtime`. Its `translateStream` contains the `CODEX_CONVERSATION_TRANSLATION_MODEL` collect-first quirk: for the conversation translation model, the stream is collected into a single `Uint8Array` via `Stream.runCollect` + `concatAudioChunks` and `onDelta` is stripped before calling `translateWithCodexRealtime`.

The OpenRouter layer owns the stream-to-clip shim (`Stream.runCollect` + `concatAudioChunks`) because the OpenRouter HTTP API is clip-only. Both `transcribeStream` and `translateStream` buffer the incoming audio stream before calling `transcribePcmWithOpenRouter` or `transcribeAndTranslatePcmWithOpenRouter` respectively.
